...
for idx = 1:N
    ...
    state_hist(:, idx) = state;
    input_hist(:, idx) = [omega_1; omega_2];
    error_hist(:, idx) = [ey; epsi];
    proj_hist(:, idx) = [ref.X_star; ref.Y_star];
    psi_ref_hist(idx) = ref.psi_des;
end
...
%% ANIMATION AND PLOTS
figure('Name', 'Tracking Errors'); clf;
subplot(3, 1, 1);
...
figure('Name', 'Vehicle Tracking'); clf;
