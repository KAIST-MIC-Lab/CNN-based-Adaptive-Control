...
\author{Myeongseok Ryu$^{1}$ and Kyunghwan Choi$^{1}$% <-this % stops a space
\thanks{*This work was supported by Korea Institute for Advancement of Technology (KIAT) grant funded by the Korea Government (MOTIE) (P0020535, The Competency Development Program for Industry Specialist)}% <-this % stops a space
...
%In \cite{CNNroadEst},\cite{CNNrobot}, CNNs are used as road condition estimator, and object detector. However the CNNs in these works, only act as a type of add-on of the controllers.
...
%\color{red}
%In addition, the tracking performance of the proposed controller is compared with the DNN-based adaptive controller, which uses predent sensory data, based on \cite{DixonDNN}.
...
\begin{itemize}
    ...
    %\item $\overset{\curvearrowleft}\prod^{n}_{l=1} A_l = A_nA_{n-1}\cdots A_1$ with positive integer $n$ denotes the right-to-left matrix product operator. $\overset{\curvearrowleft}\prod^{n}_{l=k} A_l = 1$ if $k>n$.
...
\end{array}} \right.$, where $k$ and $n$ are positive integers and $A_l$ is a matrix.
    %\item ($f\circ g)(x) = f(g(x))$ denotes the function composition operator.
\end{itemize}
...
%$\text{sgn}(\cdot)$ denotes the vector signum function. 
...
\begin{equation}
    \begin{aligned}
        % {\partial \Phi_i\over\partial W^{l_k}_{0}} 
        % &=
        % \sum_{l_i=1}^{n_{1}}
        % \bigg(
        % {\partial \Phi_i\over\partial {\Phi^C_{0}}_{(l_i,l_k)}} 
        %     \text{row}_{(l_i:l_i+p_0-1)}\phi^C_0(X)
        % \bigg) 
        % \\
         % &\hspace{125pt}\text{with }j_c=0,\hfilneg\\        
...
% , where it is also shown that the Jacobians are bounded if following assumption is satisfied, where ${\phi^C_{j_c}}'\triangleq {\partial\over\partial X} {\phi^C_{j_c}}(X)$ denotes the Jacobian of the activation function with respect to a matrix $X$.
% \begin{assum} \label{assum1}
%      $\partial \Phi/\partial C(\Phi^C_{k_c})$, $\Omega_{j_c}$ and $B_{j_c}$, $\forall j_c\in[0,\dots,k_c]$ are bounded.
% \end{assum}

% if $\partial \Phi_i/\partial \Phi^C_{j_c}$ , $X$, $\Omega_{j_c}$, $B_{j_c}$, $\phi^C_{j_c}$, and ${\phi^C_{j_c}}'$ are bounded for all $j_c\in[0,\dots,k_c]$.
...
% Note that, since the activation functions and its gradients are bounded for a bounded input, $\partial \Phi/\partial V_{j_f},\ \forall j_f\in[0,\dots,k_f]$ and $\partial \Phi/\partial C(\Phi^C_{k_f})$ are bounded if following assumption is satisfied.
% \begin{assum} \label{assum2}
%     $\Phi^C_{k_c}$ and $V_{j_f}$, $\forall j_f\in[0,\dots,k_f]$ are bounded.
% \end{assum}% ================================================

\subsection{Derivation of Adaptation Laws} 

For simplicity, the weights of the FCLs and CVLs are expressed as column vectors:
%let $\theta_F \in\mathbb{R}^{\sum_{j_f=0}^{k_f} (l_{j_f}+1) l_{j_f+1}}$ and  $\theta_C \in \mathbb{R}^{\sum_{j_c=0}^{k_c} (p_{j_c}m_{j_c}+1)q_{j_c}}$ denote weights and biases in the FCN and CNN layers, respectively, such that 
...
\end{equation}  
where
%the Jacobians of $\Phi$ respect to $\theta_F$ and $\theta_C$ are derived as 
\begin{equation}
    \begin{aligned}
...
%         \\
%         &
%         +\tilde\theta^\top \hat\Phi'^\top  {A_c^{-1}}^\top  e
%         +\tilde\theta^\top \Gamma^{-1}\rho||e|| \hat \theta
% \\
\le &
        e^\top  A_c e -k_s ||e|| + ||e|| (\Phi'_M ||\tilde \theta||  +\bar\Delta ) 
\\      
...
\begin{figure}%[!t]
    \centering
        \subfigure[]{\includegraphics[width=0.45\linewidth]{Fig1}%
        \label{fig:CNN1}}
    \hfill
        \subfigure[]{\includegraphics[width=0.45\linewidth]{Fig2}%
        \label{fig:CNN2}}
    \vfill
        \subfigure[]{\includegraphics[width=0.45\linewidth]{Fig3}%
        \label{fig:CNN3}}
    \hfill
        \subfigure[]{\includegraphics[width=0.45\linewidth]{Fig4}%
        \label{fig:CNN4}}
    \vfill
        \subfigure[]{\includegraphics[width=0.45\linewidth]{Fig5}%
        \label{fig:CNN5}}
    \hfill
        \subfigure[]{\includegraphics[width=0.45\linewidth]{Fig6}%
        \label{fig:DNN}}
...
\end{itemize}
%According to Remark.~\ref{remark:stack param}, smaller stacking time can make the input matrix finer dynamical information. 
%Corresponding results are shown in Fig.~\ref{fig:CNN1}, and Fig.~\ref{fig:CNN2}. 
%CNN2 whose stacking time is $0.01$s regulates the tracking error faster than CNN1. However, since the controller only considers shortsighted dynamics, it can be sensitive to noise and disturbance. 
%Moreover, it may lead the controller to act like a DNN-based controller that only considers current information.

%In \cite{DixonDNN}, the adaptation law was derived based on Lyapunov stability analysis. 
%By selecting $A_c=-1,\rho = 0$ the proposed adaptation law becomes the same as \cite{DixonDNN}.
%It can be observed from Fig.~\ref{fig:CNN3}-\ref{fig:CNN5} that the proposed adaptation law makes the learning phase more stable and robust while other parameters are equal.
%Effects of the parameters also can be observed, that the increased damping factor $\rho$, and farther eigenvalues of $A_c$, made CNN learn the control policy like damped system.
%Especially, in the case of $A_c$, the CNN appears that the weights and biases are not converged. 
...
\begin{equation}
    \begin{aligned}
        ...
        % \bigg(
        % {\partial \Phi_i\over\partial {\Phi^C_{0}}_{(l_i,l_k)}} 
        %     \text{row}_{(l_i:p_0+l_i-1)}\phi^C_0(X)
        % \bigg) 
        % \\
        %  &\hspace{125pt}\text{with }j_c=0,\hfilneg\\        
        {\partial \Phi_i\over\partial W^{l_k}_{j_c}} 
...
% Note that if Assumption \ref{assum1} is satisfied, the Jacobians of $\Phi$ with respect to CVL are bounded as shown in \eqref{eq. dPhi/dC}-\eqref{eq. dPhidOm}, since activation functions and its gradients are bounded for bounded inputs (i.e. $\phi^C_{j_c}$, ${\phi^C_{j_c}}'$ are bounded.).
